...
type funcChangeListener struct {
	listener func(event *ChangeEvent)
}

func (f *funcChangeListener) OnChange(event *ChangeEvent) {
	f.listener(event)
}
...
func AddChangeListenerFunc(listenerFunc func(event *ChangeEvent)) {
	listener := funcChangeListener{listenerFunc}
